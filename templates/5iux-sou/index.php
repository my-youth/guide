...
<head>
  ...
  <link rel="apple-touch-icon-precomposed" href="<?php echo $Theme?>/icon/280.png?v=1.0.1" />
  <meta name="msapplication-TileImage" content="<?php echo $Theme?>/icon/280.png?v=1.0.1" />
  ...
  <link  href="<?php echo $Theme?>/style.css?v=<?php echo $version; ?>" rel="stylesheet">
  <link rel="stylesheet" href="<?php echo $libs?>/Font-awesome/4.7.0/css/font-awesome.css">
  <link rel="stylesheet" href="<?php echo $libs?>/Layui/v2.6.8/css/layui-icon.css">
  <script src="<?php echo $libs?>/jquery/jquery-3.6.0.min.js">"></script>
  <script src="<?php echo $Theme?>/sou.js?v=<?php echo $version; ?>"></script>
...
<body>
    ...
    <div class="list closed">
        <ul>
            ...
            ?>
			<li class="title"> <i style="display: inline;"  <?php echo geticon2($category['Icon']);?>></i> <?php echo $category['name'].$property;?>  </li>
			 <?php
				foreach ($links as $link) {
					$link['description'] = empty($link['description']) ? '作者很懒，没有填写描述。' : $link['description'];
			 ?>
             <li><a rel="nofollow" href="<?php echo geturl($link); ?>" title="<?php echo $link['description']; ?>" target="_blank">
                    <img class="linkimg" src="<?php echo geticourl($IconAPI,$link); ?>" width="16px" height="16px">
    ...
    <div id="content">
        <div class="con">
            <div class="shlogo" ><h2 class="title"><?php echo getconfig($config.'title','上网，从这里开始！');?></h2></div>
            <div class="sou">
                <form action="" method="post" target="_self">
                   <?php 
                   if ($t=="b"){
                     echo'<div class="lg" style="background: url('.$Theme.'/icon/baidu.svg) no-repeat center/cover;" onclick="window.location.href=\'?t=g\';"></div>';
                   }elseif ($t=="g"){
                     echo'<div class="lg" style="background: url('.$Theme.'/icon/g.svg) no-repeat center/cover;" onclick="window.location.href=\'?t=b\';"></div>';
                   }else{
        ...
        <div class="foot" style="height: 40px;">
            <?php 
            if($is_login) { 
                echo "<li class=\"nav-item\"><a class=\"nav-link\" href=\"./index.php?c=admin&u={$u}\" target=\"_blank\"><span>后台管理</span></a></li>\n";
            }elseif ($site['GoAdmin']  ) {
                $url = './index.php?c='.($login =='login'?$login:$Elogin)."&u=$u";
                echo "<li class=\"nav-item\"><a class=\"nav-link\" href=\"$url\" target=\"_blank\"><span>登录后台</span></a></li>\n";
            } ?>
          Theme by <a href="https://github.com/5iux/sou/" target="_blank">5iux</a> .Copyright ©<?php echo date("Y") ?> <a href="https://gitee.com/tznb/OneNav">OneNav Extend</a>.  All Rights Reserved.
